...
public class FreshPromotion {
    ...
    public static void main(String[] args) {
        ...
        String [][] codeList = {{"anything","a"},{"a","b"}};
        String [] cart ={"a","a","a","c","a","b"};

        System.out.println(getWinner(codeList,cart));


    }

    static int getWinner(String[][] codeList, String[] cart) {
        ...
        while (cartIndex < cart.length && codeIndex < codeList.length) {
            ...
            String curr = cart[cartIndex];

            if ((codeList[codeIndex][0].equals("anything") || codeList[codeIndex][0].equals(curr))
                    && hasOrder(cart,cartIndex,codeList[codeIndex])) {
                cartIndex += codeList[codeIndex].length;
                codeIndex++;
            } else {
                    cartIndex++;
            }
    ...
    static boolean hasOrder(String [] cart, int index, String [] code){

        for(String s: code){

            if(s.equals("anything") || (index<cart.length && s.equals(cart[index]))){
                index++;
            }else{
                return false;
            }
